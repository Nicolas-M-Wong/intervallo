...
function sendGetRequest2(fileName) {
    const url = `/${fileName}`;

    return fetch(url, {
        method: 'GET',
        headers: {
            'Content-Type': 'text/html', // This header is mostly unnecessary as fetch automatically handles it
        }
    })
    .then(response => {
        if (!response.ok) {
        ...
        // Determine if the file is a JS file based on its extension
        if (fileName.endsWith('.js')) {
            return response.text().then(jsContent => {
                // Execute the JavaScript code in the current context
            ...
            // If not JS, assume it's an HTML file and parse it as HTML
            return response.text().then(htmlContent => {
                const parser = new DOMParser();
                const doc = parser.parseFromString(htmlContent, 'text/html');

                // Replace the contents of <head> and <body>
                document.head.innerHTML = doc.head.innerHTML;
                document.body.innerHTML = doc.body.innerHTML;
